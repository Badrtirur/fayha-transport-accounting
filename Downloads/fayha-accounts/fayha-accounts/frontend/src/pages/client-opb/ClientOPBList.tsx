...
import type { ClientOPB, Client } from '../../types';
import { clientOPBApi, customersApi } from '../../services/api';
import Pagination from '../../components/common/Pagination';
import Modal from '../../components/common/Modal';


const ClientOPBList: React.FC = () => {
  const navigate = useNavigate();
  const [entries, setEntries] = useState<ClientOPB[]>([]);
  ...
  useEffect(() => {
    Promise.all([clientOPBApi.getAll(), customersApi.getAll()]).then(([opbList, clientList]: any) => {
  ...
  const handleEdit = (entry: ClientOPB) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        const updated = await clientOPBApi.update(editingId, payload);
        setEntries(prev => prev.map(e => e.id === editingId ? updated : e));
        toast.success('Entry updated successfully!', { style: { borderRadius: '12px', background: '#10b981', color: '#fff' } });
      } else {
        const created = await clientOPBApi.create(payload);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Client Opening Balance</h1>
            <p className="text-slate-500 mt-0.5 text-sm">
              Manage client opening balance entries for the fiscal year.
            </p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card-premium p-5 bg-gradient-to-br from-emerald-50 to-emerald-100/50 border-emerald-200/50">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wider">Total Clients</p>
              <p className="text-2xl font-bold text-emerald-900 mt-1">{uniqueClients}</p>
            </div>
      ...
      <div className="card-premium p-4">
        <div className="relative w-full lg:w-72">
          ...
          <input
            type="text"
            placeholder="Search client balances..."
            value={searchTerm}
            onChange={(e) => {
        ...
        <div className="card-premium overflow-hidden">
          ...
          {filteredEntries.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-sm text-slate-500">Add a new client opening balance to get started.</p>
            </div>
          )}
      ...
      {/* Add New Modal */}
      <Modal isOpen={modalOpen} onClose={() => { setModalOpen(false); setEditingId(null); }} title={editingId ? 'Edit Client Opening Balance' : 'Add Client Opening Balance'} size="lg">
